...
apiMocker.start = function (port, callback) {
  ...
  apiMocker.expressInstances = [];

  apiMocker.options.ports.forEach(function(portConfig) {
      console.log(JSON.stringify(portConfig));
      var port = portConfig.port;
      var expressInstance;
      if(portConfig.secure) {
          var tlsOptions = {
              pfx: fs.readFileSync(portConfig.options.pfx),
              passphrase: portConfig.options.passphrase
          };
          expressInstance = https.createServer(tlsOptions, apiMocker.express);
      } else {
          expressInstance = http.createServer(apiMocker.express);
      }
      apiMocker.expressInstances.push(expressInstance);
      expressInstance.listen(port, callback);
...
apiMocker.stop = function(callback) {
  if (apiMocker.expressInstances) {
    apiMocker.log("Stopping mock server.");
    apiMocker.expressInstances.forEach(function(expressInstance) {
        expressInstance.close(callback);
    });
  }
  return apiMocker;
};
